...
import { updateGpsProvider } from '@/lib/vehicleGpsProvider'
...
export async function POST(request: NextRequest) {
  ...
  if (!body || typeof body !== 'object' || Array.isArray(body)) {
    return noStoreJson(
      { error: 'GPS update body must be a JSON object.' },
      { status: 400 }
    )
  }
  ...
  try {
    ...
    const result = await updateGpsProvider({ redis, input: body })
    ...
    logTelemetryApiError('/api/vehicle/gps/update', error)

    return noStoreJson(
      telemetryErrorJson(error, 'Failed to store GPS update.'),
      { status: 500 }
    )
  }
}
